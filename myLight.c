...
static GLfloat light0Pos[]      = {0.0, 0.0, 0.0, 0};   /* 照明の位置：(0,0,100,0) (x,y,z,w)*/
static GLfloat light0Diffuse[]  = {0.8, 0.8, 0.8, 1.0}; /* 拡散光: RGBA */
static GLfloat light0Specular[] = {0.3, 0.3, 0.3, 1.0}; /* 鏡面光: RGBA */

static GLfloat light1Pos[]      = {0.0, 0.0, 0.0, 1.0}; 
static GLfloat light1Diffuse[]  = {0.7, 0.7, 0.7, 1.0}; 
static GLfloat light1Specular[] = {0.5, 0.5, 0.5, 1.0};

static GLfloat lightAmbient[]   = {0.5, 0.5, 0.5, 1.0}; /* 環境光: RGBA */
...
myLightSet(void)
{
	...
	*/
	...
	// ライト1の属性の設定
	camera=getMyCamera(); 
	light1Pos[0]=camera->Eye[0];
	light1Pos[1]=camera->Eye[1];
	light1Pos[2]=camera->Eye[2];
